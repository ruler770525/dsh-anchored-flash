...
/**
 * The capability index: resident tools (bash / pwsh / str_replace_editor /
 * read/write/edit/glob/grep / memory / dtodo / todo_write /
 * ask_user_question / web_search / de_session / read_image) cannot cover
 * these, so the model must search and unlock them on demand. Kept in the
...
const UNLOCKABLE_INDEX = [
  ...
  'de_broadcast / de_canvas / de_notify / de_channel_send / de_session_search — 广播、画板、通知、会话搜索',
  'memory_suggest / memory_review_status / skill_manage — 记忆建议、审查与技能管理',
  'cordis_define / cordis_run / cordis_stop / cordis_inspect_* — 动态 Cordis 插件',
  'mcp__playwright__* — 浏览器自动化（Edge）',
]
...
export function apply(ctx) {
  ctx.tools.register({
    ...
    description: [
      ...
      '',
      'This session starts with a minimal resident set: bash, pwsh, str_replace_editor, read, write, edit, glob, grep, memory, dtodo, todo_write, ask_user_question, web_search, de_session, read_image, exit_plan_mode, skill_search, skill_load. Everything else is unlocked on demand through this tool.',
      '',
