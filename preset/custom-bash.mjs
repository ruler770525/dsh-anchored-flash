...
export function apply(ctx, config) {
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      // Resolve the configured Git Bash path; if it is missing (moved Git
      // install, different drive letter), fall back to a PATH lookup of
      // `bash` (WSL shim or another bash on PATH) so the tool never dies
      // because of a stale absolute path.
      let shell
      try {
        shell = await ctx.subprocess.resolveExecutable(bashPath, undefined, exec?.signal)
      } catch {
        shell = await ctx.subprocess.resolveExecutable('bash', undefined, exec?.signal)
      }
